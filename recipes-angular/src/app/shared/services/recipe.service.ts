...
export class RecipeService {
  ...
  newRecipe(recipe: Recipe): Observable<Recipe> {
    return this.http
      .post(`${this.URL}/api/recipes/add?auth=${this.token}`, recipe)
      .pipe(
        map((recipe: any) => {
          return recipe;
        }),
  ...
  editRecipe(recipe: Recipe, id: string): Observable<Recipe> {
    return this.http
      .put(`${this.URL}/api/recipes/edit/${id}?auth=${this.token}`, recipe)
      .pipe(
        map((recipe: any) => {
          return recipe;
        }),
  ...
  deleteRecipe(id: string): Observable<Recipe> {
    return this.http
      .delete(`${this.URL}/api/recipes/delete/${id}?auth=${this.token}`)
      .pipe(
        map((recipe: any) => {
          return recipe;
        }),
  ...
  likeRecipe(recipe: Recipe): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      ...
      if (localStorage.getItem('likedRecipes')) {
        ...
        const index = likedRecipes.findIndex(
          (likedRecipe: any) => likedRecipe === recipe._id
        );
        if (index !== -1) {
